...
export const ModalProvider = ({
    ...
    const [open, setOpen] = useState(def_open);
...
export function Modal({ children, open }: { children: ReactNode; open: boolean }) {
    return <ModalProvider def_open={open}>{children}</ModalProvider>;
}
...
export const ModalBody = ({
    ...
    const { setOpen } = useModal();
    useOutsideClick(modalRef, () => (setOpen(false), handleOnCLoseModal()));
        ...
        <AnimatePresence>
            {open && (
                ...
                >
                    ...
                    >
                        <CloseIcon handleOnCLoseModal={handleOnCLoseModal} />
                        {children}
                    </motion.div>
                </motion.div>
            )}
...
export const ModalContent = ({
    ...
    return (
        <div className={cn("flex flex-1 flex-col bg-background p-8 md:p-10", className)}>
            {children}
        </div>
    );
};
...
export const ModalFooter = ({
    ...
    return (
        <div className={cn("flex justify-end bg-gray-100 p-4 dark:bg-background", className)}>
            {children}
        </div>
    );
};
...
const CloseIcon = ({ handleOnCLoseModal }: { handleOnCLoseModal: () => void }) => {
    const { setOpen } = useModal();
    return (
        <button
            onClick={() => (setOpen(false), handleOnCLoseModal())}
